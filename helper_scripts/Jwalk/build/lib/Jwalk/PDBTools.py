...
def write_xl_to_txt(paths,pdb):
	...
	with open('./Jwalk_results/%s_crosslink_list.txt' % pdb[:-4],'w') as outf:
		...
		outf.write(' '.join('{0:<13}'.format(col) for col in ['Index','Model','Atom1','Atom2','SASD']))
		outf.write('\n')
		index = 1
		...
		for xl in paths:
			...
			sasd=xl[2]
			outf.write(' '.join('{0:<13}'.format(col) for col in [index,pdb,atom1,atom2,sasd]))
			outf.write('\n')
			index +=1
        
def write_xl_to_pdb(dens_map,paths,pdb):
	...
	path_coord = {}
	#print dens_map.origin
	for xl in paths:
		a = []
		for (x,y,z) in paths[xl]:
			#a.append((Vector((x*apix)+origin[0], (y*apix)+origin[1], (z*apix)+origin[2])))
...
def convert_UV_list(filename,uv):
    start_UV = []
    end_UV = []
    if uv == True:
        with open(filename) as UV_in:
            for line in UV_in:
                col = line.split("\t")
                start = col[0].split()
                end = col[1].split()
                start_UV.append((start[0],start[1]))
                end_UV.append((end[0],end[1]))
